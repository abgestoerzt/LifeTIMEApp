...
def test_start_erfordert_login() -> None:
    response = Client().get(reverse("checkins:start"))
    assert response.status_code == 302


@pytest.mark.django_db
def test_start_leitet_weiter_ohne_plan(user_a: User, user_b: User) -> None:
    ...
    response = c.get(reverse("checkins:start"))
    assert response.status_code == 302


@pytest.mark.django_db
def test_start_zeigt_formular_mit_plan(user_a: User, user_b: User) -> None:
    s = paar_erstellen(user_a, setup_kinder=False, setup_haustiere=False)
    paar_beitreten(user_b, str(s.invite_code))
