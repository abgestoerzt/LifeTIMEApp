...
def test_auswertung_erfordert_login() -> None:
    response = Client().get(reverse("results:auswertung"))
    assert response.status_code == 302


@pytest.mark.django_db
def test_auswertung_leitet_weiter_ohne_session(user_a: User) -> None:
    c = Client()
    c.force_login(user_a)
    response = c.get(reverse("results:auswertung"))
    assert response.status_code == 302


@pytest.mark.django_db
def test_auswertung_zeigt_seite_bei_beide_fertig(user_a: User, user_b: User) -> None:
